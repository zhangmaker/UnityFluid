	...
	SubShader {
		...
		Pass {
			...
			v2f vert(appdata v) {
				...
				float offsetX =
					_SteepnessOfWaves.x * dir1.x * cos(dirDotPoint1*kVec.x + _Time.y * _Speed.x) +
					_SteepnessOfWaves.y * dir2.x * cos(dirDotPoint2*kVec.y + _Time.y * _Speed.y) +
					_SteepnessOfWaves.z * dir3.x * cos(dirDotPoint3*kVec.z + _Time.y * _Speed.z) +
					_SteepnessOfWaves.w * dir4.x * cos(dirDotPoint4*kVec.w + _Time.y * _Speed.w);
				float offsetY =
					_SteepnessOfWaves.x * dir1.y * cos(dirDotPoint1*kVec.x + _Time.y * _Speed.x) +
					_SteepnessOfWaves.y * dir2.y * cos(dirDotPoint2*kVec.y + _Time.y * _Speed.y) +
					_SteepnessOfWaves.z * dir3.y * cos(dirDotPoint3*kVec.z + _Time.y * _Speed.z) +
					_SteepnessOfWaves.w * dir4.y * cos(dirDotPoint4*kVec.w + _Time.y * _Speed.w);

				float3 validObjectPos = float3(v.vertex.x + offsetX, height, v.vertex.z + offsetY);
				...
				float normalY =
					1.0f-(_SteepnessOfWaves.x * sin(dirDotPoint1*kVec.x + _Time.y * _Speed.x) +
						_SteepnessOfWaves.y * sin(dirDotPoint2*kVec.y + _Time.y * _Speed.y) +
						_SteepnessOfWaves.z * sin(dirDotPoint3*kVec.z + _Time.y * _Speed.z) +
						_SteepnessOfWaves.w * sin(dirDotPoint4*kVec.w + _Time.y * _Speed.w));
				float normalZ =
				...
				float3 validNormal = float3(normalX, normalY, normalZ);
			...
			fixed4 frag(v2f i) : SV_Target{
				...
				fixed4 col = float4(i.color.rgb, 0.7f);
